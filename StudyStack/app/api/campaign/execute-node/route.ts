  ...
  try {
    ...
    if (context.nodeType === 'exa_research') {
      try {
        ...
        const queryPrompt = `You are a Lead Generation Expert for StudyStack (overseas education consultancy).
...
Return ONLY a JSON array of 6 query strings.`;
        ...
        const analysisPrompt = `You are a senior lead-generation strategist for StudyStack. Return concise actionable markdown.
...
`;
    ...
    if (context.nodeType === 'email') {
      ...
      try {
        ...
        try {
          ...
          const paragraph1 = `Are you dreaming of studying abroad and looking for ${benefits.map((b, i) => i === benefits.length - 1 && benefits.length > 1 ? `and ${b}` : b).join(', ')}? You're in the right place. At StudyStack, we've helped over 45,000 students turn their overseas education dreams into reality.`;
  ...
  <div style='margin-top: 40px; padding-top: 20px; border-top: 1px solid #eeeeee;'>
    <p style='color: #999999; font-size: 14px; line-height: 1.5; margin-bottom: 10px;'>
      Thank you for considering StudyStack. We're committed to your success.
    </p>
