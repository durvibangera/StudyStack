...
import StudentLeadsSection from "./StudentLeadsSection";
...
export default function KanbanBoard() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      console.error("[KanbanBoard] bookings fetch error:", err);
    }
  }, []);
  ...
  // Convert WhatsApp bookings into calendar events
  const bookingEvents = bookings.map((b) => {
    const d = new Date(b.scheduledAt);
    const timeStr = d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' });
    return {
      date: d,
      title: `1:1 Session — ${b.studentName}`,
      time: timeStr,
      type: 'booking',
      desc: `${b.durationMinutes}min session via WhatsApp${b.notes ? ` · ${b.notes}` : ''}`,
      status: b.status,
      bookingId: b._id,
      phone: b.phoneNumber,
    };
  ...
  // Merged events: static + live bookings
  const scheduleEvents = [...staticEvents, ...bookingEvents];
  ...
  const eventTypeColors = {
    ...
    booking: "bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300 border-green-300 dark:border-green-500/30",
  };
  ...
  return (
    <div>
      ...
      {/* Students Tab */}
      {activeTab === "students" && <StudentLeadsSection />}
              ...
              <div className="space-y-3">
                {selectedDayEvents.map((ev, idx) => (
                  <div key={idx} className={`p-3 rounded-lg border ${eventTypeColors[ev.type] || eventTypeColors.session}`}>
                    ...
                    {ev.type === 'booking' && ev.phone && (
                      <p className="text-[10px] opacity-70 mt-0.5">📞 {ev.phone}</p>
                    )}
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[9px] font-bold uppercase tracking-wider opacity-60">{ev.type}</span>
                      {ev.type === 'booking' && ev.status && (
                        <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${
                          ev.status === 'confirmed' ? 'bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200'
                          : ev.status === 'pending' ? 'bg-yellow-200 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200'
